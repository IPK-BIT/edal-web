		...
		<div class="card-body p-0">
					...
					<table class="table w-full table-zebra">
						<thead>
							<tr>
								...
								<th>Relative Path</th>
								<th>Type</th>
								<th class="pr-4 text-right">Size</th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							{#each Object.keys(value) as key, i}
								<tr class={value[key].size === 0 ? 'bg-warning/50' : ''}>
									...
									<td class="max-w-xs truncate"
										>{value[key].webkitRelativePath || value[key].name}</td
									>
									<td>
										{#if value[key].type}
											{value[key].type}
									...
									<td class="text-muted pr-4 text-right text-sm">
			...
			{:else}
				<div class="p-6 text-center">
					<svg
						class="mx-auto mb-2 h-12 w-12 opacity-60"
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
						<path
							...
							stroke-linejoin="round"
							stroke-width="1.5"
							d="M3 15a4 4 0 004 4h10a4 4 0 000-8 5 5 0 00-9.9-1.2A4 4 0 003 15z"
						/>
					</svg>
					<div class="text-sm opacity-70">
						No files selected. Use "Select Folder" to load files.
					</div>
				</div>
			{/if}
		</div>
